...
### Dataset
* Real data on 22 colorectal cancers classified into 4 molecular subtypes, with up to 3 samples per cancer (60 samples total), generated by IMC. 39 proteins were measured simultaneously. Each pixel covers an area of 1µm2.
* **Raw pixel** data as tiff images, where every image is an 8 bit grayscale encoding the measured expression of one protein. Additionally, we provide a binarized version of the data following a protocol in [3] in the same structure.
* **Segmentation masks** as 32 bit tiff image files, where all pixels assigned to a cell are inscribed with a unique integer id.
* **Protein measurements** aggregated per cell, according to the above segmentation.
* **Cell labels** table that assigns a cell type label to every cell id from the segmentation masks.
